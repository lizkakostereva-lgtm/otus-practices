...
# Определяем представление признаков, содержащее показатели качества водителя.
# Эти признаки характеризуют успешность выполнения заказов и принятия заявок.
# Представление используется для обучения моделей оценки качества работы водителей
# и для получения признаков в режиме онлайн.
driver_quality_fv = FeatureView(
    name="driver_quality_fv",
    entities=[driver],
    ttl=timedelta(days=1),
    schema=[
        # Доля успешных конверсий (conversion rate)
        Field(name="conv_rate", dtype=Float32),

        # Доля принятых заказов (acceptance rate)
        Field(name="acc_rate", dtype=Float32),
    ],
    online=True,
    source=driver_stats_source,
...
# Определяем представление признаков, содержащее показатели активности водителя.
# Эти признаки характеризуют интенсивность работы водителя и могут использоваться
# для прогнозирования спроса, нагрузки и эффективности работы.
driver_activity_fv = FeatureView(
    name="driver_activity_fv",
    entities=[driver],
    ttl=timedelta(days=1),
    schema=[
        # Среднее количество поездок водителя за день
        Field(
            name="avg_daily_trips",
            dtype=Int64,
            description="Среднее количество поездок в день",
        ),
    ],
    online=True,
    source=driver_stats_source,
...
# Определяем представление признаков по требованию (On-Demand Feature View),
# которое вычисляет дополнительные показатели эффективности водителя
# на основе существующих признаков качества и активности.
# Эти признаки не хранятся в источнике данных, а рассчитываются во время запроса.
@on_demand_feature_view(
    sources=[driver_quality_fv, driver_activity_fv],
    schema=[
        Field(name="acceptance_gap", dtype=Float64),
        Field(name="realtime_efficiency", dtype=Float64),
    ],
)
def driver_realtime_metrics(inputs: pd.DataFrame) -> pd.DataFrame:
    df = pd.DataFrame()

    # Разница между долей принятых заказов и долей успешных конверсий.
    # Позволяет оценить расхождение между принятием и завершением заказов.
    df["acceptance_gap"] = (
        inputs["acc_rate"] - inputs["conv_rate"]
    )

    # Интегральная метрика эффективности водителя.
    # Учитывает как активность (число поездок), так и качество работы.
    df["realtime_efficiency"] = (
        inputs["avg_daily_trips"] * inputs["acc_rate"]
    )
